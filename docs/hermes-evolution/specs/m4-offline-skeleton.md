...
>
> **状态**：草稿中（2026-06-12，C-rev19 RED-only fix round：parent C-rev18 / `316531ee`。本轮闭合 7 REDs inline（CORR-1..6 + COH-1）+ 6 YELLOWs 路由至 sibling CF [`m4-carry-forward.md`](./m4-carry-forward.md) §8（chronological per-round sub-section per sibling 约定），无新 §0.3 decision row（RED-1/RED-2 通过 inline-amend 既有 #123/#124）。**前轮 C-rev18 FINAL SWEEP**：执行 **CF-C-rev15-5 close-path (a)** —— §12 整章物理抽离至 sibling file [`m4-carry-forward.md`](./m4-carry-forward.md)，spec §12 退化为 3-line pointer，per-round sub-sections (12.2–12.7) 在 sibling file 重组为 chronological numbered sections (§2–§7)；3 条 CF 共同闭合（CF-C-rev15-5 path-(a) 执行 + CF-C-rev16-1 path-(a) co-closure clause + CF-C-rev17-1 §12-relocation co-closure clause），sibling file 头部 + 各 entry header 加 `[CLOSED C-rev18 via close-path (a)]` marker（body 保留作 audit trail）。同轮一次性起草 §7–§14 substance：§7 Judge rubric + κ ≥ 0.6 calibration / §8 PR-only deploy 5-段 PR body + label / §9 4-stage redaction pipeline + per-row manifest / §10 9 不变量编号 + enforcement point / §11 8 项 deferred-to-M5 / §13 3-line pointer to §0.3 / §14 7 项 M5 继承契约。§0.3 新增 5 条决策 #123–#127（rubric default weights / `RUBRIC_PASS_THRESHOLD` / calibration corpus size 25–35 / κ ≥ 0.6 threshold / redaction stage ordering），cap 内（C-rev19 / RED-1+RED-2 把 #123/#124 由 4-axis 0–10 整数 inline-amend 至 3-axis [0.0,1.0] 连续浮点对齐 §3.3 canonical schema）。§12 未关闭 entry 数：相对 C-rev17 ≈ 16 → C-rev18 ≈ 13（CF-C-rev15-5/16-1/17-1 关闭）；§12 现以 sibling file 形态承载，CF-C-rev11-1 hard threshold 重设基线。§0–§5 已 4-reviewer 收敛 PASS，§6 C-rev14/15/16/17 reviewer-fix 四轮落地，§6 仍待 4-reviewer C-rev17 收敛验收；§7+ C-rev18 起草到位，待 4-reviewer round E 验收。前轮 C-rev17 闭合 6 inline RF + 1 §12.7 entry（CF-C-rev17-1）；C-rev16 闭合 7 inline RF + 1 §12.6 entry（CF-C-rev16-1）；C-rev15 闭合 3 must-fix YELLOW（C-Y1/2/3）+ 6 advisory CF（CF-C-rev15-1..6）+ 决策 #122；C-rev14 闭合 4 RED + 12 must-fix YELLOW（决策 #115–#121）；C-rev13 §6 7-subsection draft（决策 #109–#114）；C-rev11 闭合 1 RED + 6 YELLOW + 2 tighten（决策 #107 / #108 + CF-C-rev11-1/2）。
>
...
| 123 [NEW C-rev18 / §7.2] [AMENDED-INLINE C-rev19 / RED-1] | **Rubric default weights spec-lock**：`RubricWeights` 默认权重锁定为 `process=0.4 / output=0.4 / token=0.2`（对齐 §3.3 既有 3-axis canonical schema；C-rev18 早期草拟的 4-axis `correctness/style_match/parsimony/safety` 与 §3.3 `RubricWeights._sum_to_one` 三字段 validator 冲突，C-rev19 / RED-1 inline-amend 收敛到 3-axis）。Sum-to-one 由 `RubricWeights._sum_to_one` 验证（决策 #84）。Process / output 等权 0.4 反映两者并列首要（流程合规 + 输出正确两轴皆不可少），token 0.2 反映经济度作 soft preference。Spec-locked，运行时不可调（与决策 #111 spec-lock 哲学一致）；M5 添加 4th axis（如 semantic fidelity）时一并 update 本决策 + calibration corpus + κ 重跑。Safety intent **不**作为 rubric 维度承载 —— 移至 §6.1 gate-1 内部 pass/fail veto 通路（详 §7.2 prose）；adversarial safety axis 推迟至 M5（§11）。**Alternative 未采纳**：(a) 等权 0.33 × 3 — 失去"token 经济是 soft preference"的层级；(b) 运行时可调权重 — silent-bypass 风险，与决策 #111 spec-lock 哲学冲突 | §7.2 | C-rev18 §7.2 起草触发；C-rev19 / RED-1 inline-amend：4-axis → 3-axis 对齐 §3.3 canonical schema |
| 124 [NEW C-rev18 / §7.2] [AMENDED-INLINE C-rev19 / RED-2] | **`RUBRIC_PASS_THRESHOLD=0.6`**：record-level aggregated rubric scalar ≥ 0.6 视为 pass，进入 gate 1 `tier_a_rate` / `tier_b_rate` 计算。Scoring 为 continuous float in [0.0, 1.0]（对齐 §3.3 `RubricScore` 字段 `Field(ge=0.0, le=1.0)`；C-rev18 早期草拟的 "0–10 integer" 与 §3.3 schema 不兼容，C-rev19 / RED-2 inline-amend 收敛到 [0.0, 1.0] 连续浮点）。Spec-locked，与 §6.1.1 既有 gate threshold（决策 #111）同处理。0.6 对应"3-axis × default weight 平均 +约 1σ" 的 "promising 但不完美" 判定边界。**Alternative 未采纳**：(a) 0.5 — "average" 视为 pass 过宽，候选 mediocre but average 中 OK 即过；(b) 0.7 — 严苛到让多数 valid candidate 被 reject，gate 1 trivially-low pass rate 破坏 GEPA 学习 signal | §7.2 | C-rev18 §7.2 起草触发；C-rev19 / RED-2 inline-amend：0–10 integer → [0.0, 1.0] 连续浮点对齐 §3.3 schema |
...
class Gate(ABC):
    ...
    #    新登记 entry 跟踪。
    #    ── C-rev17 / RF-6 governance guard（Arch-2 + Scope-5 paired）──
    #    如果 M5 或更晚 milestone 再向本 docstring 追加同类
    #    self-referential trigger note（即「关于 ABC 自身何时应当
    #    restructure」的 forward-trigger 评论），该 note 必须先在 §12
    #    register 一条 CF entry；直接落 docstring 视为 governance violation。
    #    （本 Note 4 自身是首条 self-referential 元评论，由 CF-C-rev17-1
    #    forward-guard；第二条到达即触发 docstring → 专用 governance 模块
    #    的 housekeeping 迁移。）
...
# Derivation (RF-6 / Scope-4; C-rev17 / RF-1 dimensional clarity):
#   = ceil(TIER_C_FLOOR=5 records)
#   * PER_RECORD_TIMEOUT_S=30 s/record
#   * SLACK_FACTOR≈4
#   * 1000 ms/s
#   = 600_000 ms (= 600 s = 10 min)
# Revisit at M5 if any input invariant changes.
GATE_TIMEOUT_MS_HARD: int = 600_000
```

**Forward-markers**（C-rev16 / RF-5 / Arch-3 + RF-6 / Scope-4 lifted-from-advisory）：
- **Layering**：此常量当前 co-locate 在 `gates/test_pass.py` 是 M4 唯一 long-running gate + harness post-hoc warn 的 sole-consumer 妥协；M5 gate-4 (LLM-judge) 引入即触发迁至 `nanobot/evolve/_constants.py`，原路径保留一个 milestone 的 deprecation re-export。**Owner（C-rev17 / RF-4 / Arch-4）**：**M5 round-A Arch reviewer (architecture-strategist) is the owner of evaluating whether `GATE_TIMEOUT_MS_HARD` should migrate from `nanobot/evolve/gates/test_pass.py` to a shared `nanobot/evolve/gates/_constants.py` (or equivalent shared module). Migration fires when M5 introduces ≥1 additional gate that needs to read this constant.**
- **Derivation cross-link**（C-rev17 / RF-1 dimensional clarity）：`600_000 ms = ceil(TIER_C_FLOOR=5 records) × PER_RECORD_TIMEOUT_S=30 s/record × SLACK_FACTOR≈4 × 1000 ms/s`（= 600 s = 10 min）；四个因子分别由 §1.1（Tier C floor records 数，决策 #120 precondition assert）/ 本节 `PER_RECORD_TIMEOUT_S` 秒/record 量纲 / Sourcing 段 slack rationale / 显式 ms↔s 单位换算锚定。任一输入变更需同步重新计算并在 M5 retro 评估 SLACK_FACTOR 是否仍合理。
...
### 7.2 Rubric dimensions（3 axes）

每个 judge 对每条 record 在三条 axis 上各打 [0.0, 1.0] 连续浮点分（schema 对齐 §3.3 `RubricScore` 字段 `Field(ge=0.0, le=1.0)`，决策 #84 canonical 类型）：

1. **`process`**：候选 skill 中间推理 / 流程步骤的合规度（process compliance）—— 候选是否按既有 skill 风格组织 steps、是否含错误信息 / 残留 placeholder / 未完成 step、命名注释错误处理形态是否与仓库既有 skill 一致。本 axis 承接早期 4-axis 设计中 "correctness 流程层" + "style_match" 的语义合并，反映"判 judgment of intermediate reasoning steps quality"。
2. **`output`**：候选 final output 对 `record_input` 所述任务的功能正确性（output correctness）—— 输出是否解决了任务、与 reference output（若存在）的语义等价度。本 axis 承接早期 4-axis 设计中 "correctness 终态层" 的语义。
3. **`token`**：候选 skill 实现的 token 经济度（token economy / parsimony）—— SKILL.md 行数 / step 数 / 抽象层数是否与任务复杂度相称。与 gate 2 的 `SKILL_LINE_HARD_CAP=400` 硬上限正交：gate 2 是 hard cut，本 axis 是 soft preference（候选 380 行 pass gate 2 但 axis 得 0.4 表达"虽过 gate 但过冗"）。本 axis 承接早期 4-axis 设计中 "parsimony" 的语义。

**Safety axis intent 处置**：早期 4-axis 草拟含独立 `safety` axis（cooperative-threat-model self-report），本轮 (C-rev19) 收敛到 3-axis 后，safety intent **不**作为 rubric 维度承载 —— 安全风险信号通过 §6.1 gate-1 内部的 pass/fail veto 通路捕获（candidate 触发 shell-exec without quoting / 未脱敏 secret echo / 规避 sandbox 等 → 直接判 record fail，不进入 rubric 评分加权）；adversarial 威胁模型与 dedicated safety axis 推迟至 M5（§11，gate-4 SemanticFidelityGate 引入时一并评估第 4 axis 形态）。

**Aggregation**：单 record 单 judge 的 3 axis 经 `RubricWeights` 加权得 scalar score ∈ [0.0, 1.0]（决策 #84 `RubricWeights` 作为 canonical 类型），sum-to-one 不变量由 `RubricWeights._sum_to_one` 验证（决策 #84）。Default weights：`process=0.4 / output=0.4 / token=0.2`（对齐 §3.3 `RubricWeights` field defaults；锁入 §0.3 决策 #123 [AMENDED-INLINE C-rev19 / RED-1]）。Judge pool 聚合走 median（决策 #81 奇数池要求即此原因），rubric-level scalar 进入 gate 1 的 `tier_a_rate` 计算 —— record 视为 "pass" 当且仅当 aggregated scalar ≥ `RUBRIC_PASS_THRESHOLD=0.6`（continuous float in [0.0, 1.0]，spec-locked，§0.3 决策 #124 [AMENDED-INLINE C-rev19 / RED-2]）。
...
- **Agreement metric**：判定 judge 与人类标签 agreement 时使用 **Cohen κ**，逐 axis 计算 then mean across axes（取均值以避免单 axis outlier 拉偏整体判定）。Cohen κ 选择理由：rubric axes 是 [0.0, 1.0] 连续浮点，κ 计算时按固定 bins 离散化（例 0.0–0.33 / 0.33–0.66 / 0.66–1.0 三分位）后计算，比 Pearson correlation 更适合捕捉"random agreement"的剥离；agreement threshold **κ ≥ 0.6**（"substantial agreement" per Landis & Koch 1977 标准），低于即 calibration FAIL（spec-locked，§0.3 决策 #126）。
...
## Rollback plan
`git revert <sha>`  # one-line revert; squash-merge per §8.4 makes <sha> a single-parent commit; no schema migrations expected for M4 candidates
```

`<sha>` 在 PR squash-merged 后由 GitHub 提供（single-parent commit hash）；PR description 在 merge 时 amend（rollback line 仅在 merge 后被 webhook / follow-up tool 填实，body 在 PR open 时占位）。详 §8.5 rollback precondition。
...
- **Branch protection**：`main` 上 require 上述三 label + status checks，由 admin 在 GitHub repo settings 中配置；spec 仅声明契约，配置步骤记入 implementation plan。
  - **Squash-merge mandate**：Branch protection MUST 设置为 `Allow squash merging` only；`Allow merge commits` 与 `Allow rebase merging` MUST 关闭。Squash merge 产生 single-parent commit，让 §8.5 rollback contract（`git revert <sha>` 单行）在 GitHub 默认 merge 形态下工作 —— 默认 "Create a merge commit" 产生 2-parent merge，`git revert` 不带 `-m <parent>` 会失败，破坏 PR template 发布的 rollback 命令可执行性。

### 8.5 Rollback contract

- 每个 evolve PR body 内（§8.2）含 `git revert <sha>` one-liner，**仅一行**，不带额外 cleanup 步骤；M4 阶段 candidate 仅修改 skill .md 文件，无 schema migrations / data backfill，revert 即足够回退。
- **Precondition**：PR 必须按 §8.4 squash-merge mandate 进行 squash-merged（single-parent commit），rollback 命令才可执行 `git revert <sha>`。若 non-squash merge（2-parent merge commit）somehow 落地（branch protection bypass / admin override），rollback 退化为 `git revert -m 1 <merge-sha>`，且 PR author MUST 被 page 触发 branch-protection audit（违反 §8.4 mandate）。
...
2. **Provider-key scrub**：detect `sk-[A-Za-z0-9]{20,}` (OpenAI) / `sk-ant-[A-Za-z0-9-]{20,}` (Anthropic) / `ghp_[A-Za-z0-9]{36}` / `github_pat_[A-Za-z0-9_]{40,}` / `AKIA[0-9A-Z]{16}` (AWS) 等已知 key 前缀；匹配项替换为 `<REDACTED:apikey:<provider>>`。Anthropic API keys use the `sk-ant-` prefix；the `claude-` prefix is Anthropic's MODEL identifier (e.g. `claude-3-5-sonnet`) 而非 key 前缀，MUST NOT 被本 stage redact，否则会破坏 candidate 输出中合法的 model 名引用。
...
6. **Reproducibility tagging**：每条 `GateResult` MUST 在 model 上直接 populate 4-tuple `(gate_name, candidate_hash, baseline_hash, timestamp)`（4 字段全部为 §3.6 schema 必填）。对 gate-1（test-pass，judge-driven），`evidence: dict[str, str] | None` 字段 MUST 额外承载 stringified `eval_corpus_hash` / `judge_model_id` / `judge_corpus_hash`（gates 2/3 把这三键设为 None 或省略 —— gate-2 size 与 gate-3 cache-compat 不消费 corpus / judge）。Run manifest（§3.7）聚合 per-gate `evidence` dict 加 4-tuple 形成 single reproducibility record。Enforced by harness `_run_gates` 的字段完整 check + 每个 gate 的 `evaluate()` return contract。
...
以下 8 项为 M4 **明确不做**、留给 M5（或更晚 milestone）的事项。每项一段说明 deferred 原因与 trigger。
...
- **Judge rubric 第 4 axis（semantic fidelity + dedicated safety）**：§7.2 3-axis rubric (process / output / token) 不含 reference-output 与 candidate-output 的 semantic equivalence 单独 axis（隐含于 `output`），亦不含 dedicated safety axis（M4 safety intent 走 §6.1 gate-1 内部 veto，非 rubric 维度）。M5 gate-4 SemanticFidelityGate 引入时需添加 semantic fidelity axis（及评估是否同时引入 dedicated safety axis），伴随 calibration corpus update + κ 重跑（§14 rubric extension）。
...
- **Gate ABC 继承契约**：M5 gate 4 / gate 5 MUST subclass `Gate`（§3.6），声明 `NONDETERMINISTIC: ClassVar[bool]` per 决策 #117（gate-4 LLM-judge 必为 `True`，gate-5 human-async 视实现而定），并通过 `__init_subclass__` hook 自动 append 至 `Gate._subclasses` registry（决策 #122）。Subclass MUST 实现 `evaluate(candidate, baseline) -> GateResult`（按 §3.6 schema 返回，含 §10 不变量 6 所述必填字段）。Subclass 不得新增 abstract method（破坏 LSP）；新行为通过 ClassVar opt-out（如 `LONG_RUNNING` per CF-C-rev15-3 future close criterion option b）或 hook method (`pre_evaluate` / `post_evaluate`) 引入。**STRUCTURED_KWARGS / MUST_PRECEDE 边界澄清**：这两个 registry 是 `EvolveError` mixin（异常类）的 ClassVar 契约（决策 #95 / #99），用于异常构造 kwargs-only 强制与 dispatch handler-order 拒绝；**不**适用于 `Gate` subclass —— Gate ABC 仅承接 `NONDETERMINISTIC`（#117）+ `_subclasses`（#122）两个 ClassVar 形态。M5 gate 实现者不应在 Gate subclass 上声明 `STRUCTURED_KWARGS` / `MUST_PRECEDE`（无运行时消费者，违反 ABC 单一职责）。
