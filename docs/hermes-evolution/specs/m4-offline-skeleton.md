...
7. **W-bucket 标记约定（C-rev10 / 决策 #103 / Coh-Y3）**：`W1` / `W2` / ... 等 W-bucket 标记指代**单一** C-rev<N> 轮次内离散的 reviewer-finding 闭合分组；编号**每轮重置**，对应轮次的 commit message body 中记录每个 bucket 的具体 finding-id 列表。W-bucket 标记**不是**独立的决策 ID，仅为轮次内闭合追踪所用 —— 决策日志中引用形式恒为 `C-rev<N> / W<bucket>` 全限定（以避免跨轮次误识）。
...
2. 若 `output_dir` 存在，按以下步骤执行：
   ...
   c. **Atomic swap**（契约层；实现细节移至 `nanobot/evolve/_atomic_swap.py` helper —— 决策 #98 / Z1）：
      ...
      **(c) Test contract（W5 / C-rev9 / 决策 #98 amend；C-rev10 / Corr-4/5/6 强化）**：M4 plan 期在
      ...
      - **`test_atomic_swap_postcondition`**（C-rev10 / Corr-4/5/6 强化）：
        ...
        **Parametrization（C-rev10 / Corr-5）**：MUST 用 pytest 双分支
        ...
        def test_atomic_swap_postcondition(simulate_renameat2_success, ...): ...
        ```
        ...
        * 若 helper 返回 `False`（`simulate_renameat2_success=False`，
        ...
        return `False` 但只完成一次 rename" 的中间态 leakage。

        **Inode-identity postcondition assertion（C-rev10 / Corr-6）**：除
        rename 计数外，本 test MUST 同时机械化 contract clause 5（postcondition：
        swap 完成后 `src` 与 `dst` 路径各自指向对方调用前的 inode）。Pre-call
        record + post-call assert：
        ```python
        # Pre-call:
        src_ino_before = os.stat(src).st_ino
        dst_ino_before = os.stat(dst).st_ino
        # ... call atomic_swap(src, dst) ...
        # Post-call:
        assert os.stat(src).st_ino == dst_ino_before
        assert os.stat(dst).st_ino == src_ino_before
        ```
        让 contract clause 5（inode swap postcondition）成为 CI-enforced
        observable 而非仅靠 clause 6 的 count-proxy 间接保护。两个分支
        （renameat2 路径与 fallback 路径）都 MUST 满足 inode-swap 后置；
        否则 fallback 实现只完成 "覆盖" 而未真"互换" 的 silent regression
        会被本断言直接拦截。
      ...
      三条 assertion（rename 计数 × 2 分支 + inode-identity × 2 分支 +
      precondition raise）共同把 W1 重构产物从 "spec 描述层契约" 提升为
      "CI 拦截层契约"，防止后续实现重构破坏 contract clause 4 / 5 / 6 而
      spec 文本无 fail-loud 通道。
      ...
      * **Fixture A — 异常路径（staging build OSError）**：在 step 2a（staging
        ...
        Assertion：异常向上 propagate 后，**第二个**子进程对相同
        `<output_dir>.lock` 路径执行 `flock(LOCK_EX | LOCK_NB)` 在 100ms 内
        SHOULD 立即成功（说明前次锁已 release）。fixture 实现可用
        `multiprocessing.Process` + `multiprocessing.Event` 同步。
...
# C-rev10 / Corr-1：`manifest_path: "Path"` 与 `offending_path: "Path | None"`
# 是 string-literal 前向引用；为让 `typing.get_type_hints(cls)` /
# `inspect.signature(cls.__init__, eval_str=True)` 在 introspection helper
# 中能成功解析，必须在 TYPE_CHECKING 块内提供 `Path` 的实际定义。运行时不
# 触达此 import（pathlib 不进入 evolve "no-extra" 探针的依赖闭包要求；
# stdlib only），但保留 TYPE_CHECKING 形态是为了 keep "no-MRO" /
# "no-runtime-side-effect" 模块导入语义干净。
if TYPE_CHECKING:
    from pathlib import Path
...
class EvolveError:
    ...
    仅承担两个 invariant 角色：
      ...
      2. **`STRUCTURED_KWARGS` 运行时 backstop**：`__init_subclass__` 在导入期对
         任何声明 `STRUCTURED_KWARGS: ClassVar[frozenset[str]]` 的子类校验其
         `__init__` 的 keyword-only 参数集合**严格等于** `STRUCTURED_KWARGS`；
         不一致即 `TypeError` 阻断导入。
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if kw_only != set(declared):
            raise TypeError(
                f"{cls.__name__}: STRUCTURED_KWARGS={set(declared)!r} mismatches "
                f"__init__ keyword-only params={kw_only!r}; declared frozenset MUST "
                f"equal ctor kw-only params (W3 / C-rev9 / 决策 #99 amend)."
            )
