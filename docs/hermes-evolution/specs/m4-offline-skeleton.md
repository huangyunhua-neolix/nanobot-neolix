...
2. 若 `output_dir` 存在，按以下步骤执行：
   ...
   c. **Atomic swap**（契约层；实现细节移至 `nanobot/evolve/_atomic_swap.py` helper —— 决策 #98 / Z1）：
      ...
      **(c) Test contract（W5 / C-rev9 / 决策 #98 amend）**：M4 plan 期在
      ...
      - **`test_atomic_swap_postcondition`**：monkeypatch 同时 stub `ctypes` 层
        的 `renameat2` 调用与 `os.rename`，记录二者 invocation 次数与参数。
        断言两条互斥分支：

        * 若 helper 返回 `True`：观察到 **exactly 1** 次成功的 `renameat2`
          调用 AND **0** 次 `os.rename` 调用。
        * 若 helper 返回 `False`：观察到 **0** 次成功的 `renameat2` 调用
          （fallback 触发 errno 抛出在 mock 中以 `OSError(EINVAL)` 等模拟）
          AND **exactly 2** 次 `os.rename` 调用（fallback path 的 staged
          two-step swap）。
      ...
      两条 test 共同把 W1 重构产物从 "spec 描述层契约" 提升为 "CI 拦截层契约"，
      防止后续实现重构破坏 contract clause 4 / 6 而 spec 文本无 fail-loud 通道。
...
class EvolveError:
    ...
    中（多继承形态 `class Foo(EvolveError, ValueError):`）。
    """
