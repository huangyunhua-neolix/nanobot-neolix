...
- **验证机制（R9 fix YEL-FEAS-R8-3）**：本仓库 CI 当前为 POSIX-only matrix；R8-1 / R8-1b / R8-2 通过本地 Windows VM 或 `pytest.mark.skipif(sys.platform != "win32", reason="POSIX matrix; Windows-only guard")` 装饰的单元测试块手动验证；不要求扩展 CI matrix 至 Windows。M2 不引入 Windows CI runner（在 R9-1 仍为 POSIX 上跑）。
