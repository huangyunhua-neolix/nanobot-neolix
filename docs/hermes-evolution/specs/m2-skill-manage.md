...
class SkillManageTool(Tool, ContextAware):
    ...
    Construction-time keyword `provenance_tag: str = "agent"` controls what
    is written into `created_by` / `patched_by` frontmatter. Each scope's
    registry-builder injects the tag explicitly:
      - main agent → `"agent"` (default)
      - subagent  → `f"subagent:{task_id}"` (set in SubagentManager._build_tools)
      - dream     → `"dream"` (set in MemoryStore.build_dream_tools, §6.1)
...
- **Main agent runtime** —— 主 agent 的 ToolRegistry 构造路径注册 `SkillManageTool(provenance_tag="agent")`（具体注入点在 plan 阶段确定，本 spec 不锁文件名行号）。
- **Subagent** —— `SubagentManager._build_tools` (`nanobot/agent/subagent.py:130-149`) 在 `ToolLoader().load(...)` 之后追加 `registry.register(SkillManageTool(workspace=..., telemetry=..., provenance_tag=f"subagent:{task_id}"))`。
- **Dream** —— `MemoryStore.build_dream_tools()` 显式注册带 `provenance_tag="dream"`（§6.1）。

> M2 不引入 ToolLoader scope-filter 机制（避免发明新基础设施）。哪个 scope 装载本工具，由该 scope 的 registry-builder 显式决定。
...
#### `delete` 顺序协议（R2 fix — 解决"删 `.lock` 时另一进程正持有"竞态）

`<name>/.lock` 物理上位于 `<name>/` 目录内，删整个目录会撤掉别人正在用的锁文件。下面是显式协议（实施 plan 必须按此顺序写）：

1. 取进程内 `_skill_inproc_locks[name]: threading.Lock`（懒创建）。
2. 取 `FileLock(<workspace>/skills/agent/<name>/.lock)`（默认 timeout 1.0s；超时 → `concurrency_timeout`）。
3. 进入 filelock 上下文后，**重新检查** `SKILL.md` 是否仍存在；若已不存在（被并发 delete 抢先了），按幂等返回 `not_found`，跳到第 8 步。
4. `unlink SKILL.md`。
5. 退出 filelock 上下文（`filelock` context manager 自动释放 advisory lock）。
6. `unlink <name>/.lock`（若 ENOENT 忽略——可能没人取过这把锁、文件根本没创建过）。
7. `rmdir <name>/`（要求目录已空；若有意外残留如 `.DS_Store`，记 WARN log，不强制 rm，留给运维）。
8. 释放 `threading.Lock`。

**正确性论证**：在第 4 步与第 6 步之间，任何并发 acquirer（被第 2 步阻塞的请求）会在我们退出 filelock 后获得新锁——但它们 pipeline 第一步就是"读 SKILL.md"，会立即看到文件不存在，按 §4.5 返回 `not_found`，安全 no-op。第 6 步的 `.lock` 文件即使被并发 acquirer 重新创建也无所谓——`filelock` 用的是内核 `flock`/`fcntl` advisory lock，磁盘上的 `.lock` 文件只是个稳定句柄、不是锁状态本身。
...
# rejection
{
    ...
    "error_code": "tier_locked",  # tier_locked | name_exists | not_found | search_ambiguous | search_missing | rate_limited | invalid_name | concurrency_timeout | io_error
}
```
...
- 保留名集合（Y6）：`{"agent", "user", "builtin", "hub", ".lock"}`——前四个是 origin tier 命名空间（避免与 M1 §10.1 风险表第 5 行"user 把 origin 名当 skill 名"反向触发，并预留未来 `hub` tier）；`.lock` 是 §8.1 per-skill 锁文件名，避免目录命名冲撞。
...
- **计数器存放位置**（Y1 fix）：`RuntimeState._runtime_vars["skill_manage_mutations"]: int`（`nanobot/agent/tools/runtime_state.py:52` `_runtime_vars` 是 `dict[str, Any]`；现有 `SelfTool` 已用同款 dict 存 LLM-side scratchpad，参考 `nanobot/agent/tools/self.py:340-349`）。`SkillManageTool` 在 set_context 时拿到 `runtime_state` 引用（同 `SelfTool` 的注入路径），每次 verb 成功后做 `_runtime_vars["skill_manage_mutations"] = _runtime_vars.get("skill_manage_mutations", 0) + 1`。
- **计数器重置点**：在 `AgentRunner.run()`（`nanobot/agent/runner.py:275`）入口、`_run_core` 进入主 loop 之前一次性重置 `runtime_state._runtime_vars["skill_manage_mutations"] = 0`；具体调用点由实施 plan 在 `runner.py` 的 turn-init 段落落实，本 spec 不锁行号（runner 内部还在演化）。
- **Dream 与主 agent 共享与否**：**不共享**。Dream 走 `MemoryStore.build_dream_tools()` 构造的独立 ephemeral `AgentRunner`（M1 §6 已锁），该 runner 拥有自己的 `RuntimeState`/`_runtime_vars`，因此 Dream turn 内的计数器与主 agent turn 计数器物理上是两个独立 dict——Dream 一 turn 内最多还是 5 次（同样 default），但与主 agent 的 5 次额度独立。这是 Y1+Y2 的联合解：单一计数器存储模型 + 通过 RuntimeState 实例隔离自然解决"Dream 是否分享主 agent 配额"的歧义。
