...
>
> **状态**：设计已锁定（2026-06-11，Q1–Q13 决策全部 approved；R3 patch 已合入：14 RED + 12 YELLOW 全部 resolved；R4 patch 已合入：2 RED + 17 YELLOW 全部 resolved；R5 patch 已合入：7 residual YELLOW 全部 resolved；R6 patch 已合入：2 RED + 12 YELLOW 全部 resolved，convergence target zero red/yellow）。
>
...
- **协议**（仅 `create`）：
  ...
  2. **acquire** create-lock 前，先按 §3.7.1 lock-path symlink-defense 执行 `is_symlink()` precheck 与 fd-mode `os.open(O_NOFOLLOW)` 准备；通过后再 acquire `<workspace>/skills/agent/.create.lock`（默认 timeout 1.0s；超时 → `concurrency_timeout`）。
...
3. **R6 修订（YEL-DI-1 / YEL-SEC-1）—— fd-mode lock 关闭 TOCTOU**：`is_symlink()` precheck 与 lock acquisition 之间存在 TOCTOU window（attacker 可在 precheck 后、open 前替换为 symlink）。`filelock.UnixFileLock` 实测使用 `open(path, "a")`，**会 follow symlink**——即 R5 spec 中"O_NOFOLLOW-equivalent fd acquisition"的描述是**错误的**。M2 必须自行用 fd-mode 构造关闭 TOCTOU：实施 plan **必须** `os.open(<lock_path>, os.O_RDWR | os.O_CREAT | os.O_NOFOLLOW | os.O_CLOEXEC, 0o600)` 拿到 fd，再委托给 `filelock` 或直接 `fcntl.flock(fd, LOCK_EX)`。`O_NOFOLLOW` 是 syscall 级守护：当 path 是 symlink 时 `os.open` 抛 `OSError(errno.ELOOP)` 而非静默 follow，TOCTOU 在内核层关闭。
4. **实现要求**：lock-file fd 必须以 `O_NOFOLLOW | O_CREAT | O_CLOEXEC` 打开，再交给 `filelock` 或直接 `fcntl.flock`。这样 is_symlink precheck（fast-path）+ 内核级 `O_NOFOLLOW`（syscall-level 兜底）构成真正 defense-in-depth；is_symlink 是快速失败路径，`O_NOFOLLOW` 是 syscall-level final guard。Windows fallback：缺 `os.O_NOFOLLOW` 时（参见 §4.6.1 platform gating）依赖 `is_symlink` precheck + `Path.resolve(strict=True).is_relative_to(skills_agent_root.resolve(strict=True))` 兜底。
5. 失败路径不 bump，不持任何下游锁，直接返回 `error_code = "PATH_ESCAPE"`（`is_symlink` 命中）或 `ATOMIC_WRITE_FAILED`/捕获后转译为 `PATH_ESCAPE`（`O_NOFOLLOW` 触发 `OSError(errno.ELOOP)`）。

§10.6 R5 增补 test gate："planted symlink at `<workspace>/skills/agent/.create.lock` → `PATH_ESCAPE` reject before filelock acquire" 与"planted symlink at `<workspace>/skills/agent/<name>/.lock` → 同款 reject"。R6 增补 test gate："Lock-path symlink defense — 注入 `<workspace>/skills/agent/.create.lock` 为指向外部文件的 symlink，调用 create；assert `os.open` 抛 `OSError(errno.ELOOP)` 而非静默 follow（即使 is_symlink precheck 被绕过）。"

决策 Log #67-A（R5）：lock 文件路径自身的 symlink 校验，与 §4.6.1 SKILL.md 路径校验同款合同。R6 修订：lock-path defense 不再依赖 `filelock` 库内部行为（pure-Python 实现 follows symlinks）；M2 plan 必须自行 `os.open(O_NOFOLLOW)` 取 fd 后委托给 lock 实现。详见 §13 Decision #69。
...
| 69 | 2026-06-11 (R6) | lock-path defense 必须自 `os.open(O_NOFOLLOW \| O_CREAT \| O_CLOEXEC)` 取 fd 再委托给 `filelock` 或直接 `fcntl.flock`；不依赖 `filelock` 库内部行为 | A: fd-mode lock（采纳） / B: 信任 filelock 库内部 O_NOFOLLOW（R5 误判，R6 拒绝） | `filelock.UnixFileLock` 实测 `open(path, "a")` follows symlinks；R5 spec 中"O_NOFOLLOW-equivalent fd acquisition"描述错误；TOCTOU 必须在 syscall 层关闭（详见 §3.7.1 R6 修订与 YEL-DI-1 / YEL-SEC-1） |
