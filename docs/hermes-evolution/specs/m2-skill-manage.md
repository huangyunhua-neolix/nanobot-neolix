...
>
> **状态**：设计已锁定（2026-06-11，Q1–Q13 决策全部 approved；R3 patch 已合入：14 RED + 12 YELLOW 全部 resolved；R4 patch 已合入：2 RED + 17 YELLOW 全部 resolved；R5 patch 已合入：7 residual YELLOW 全部 resolved，convergence target zero red/yellow）。
>
...
- **协议**（仅 `create`）：
  ...
  1.5. **workspace-level mkdir-on-first-create（R5 fix YEL-R5-1）**：在 acquire create-lock **之前**，执行 `os.makedirs(<workspace>/skills/agent/, exist_ok=True)`（race-safe、idempotent）。`filelock.FileLock(...).acquire()` 要求 lock 文件的父目录已存在，否则抛 `FileNotFoundError`；新 workspace 首次 `create` 时该父目录不存在，必须先 `makedirs` 才能进 step 2。`exist_ok=True` 保证多进程并发首次 create 时各自的 `makedirs` 调用都成功（最先完成的真建目录，后续的 no-op）。本步骤覆盖 workspace-level 父目录创建；§8.8 ghost-dir 重建中提到的 mkdir-on-first-create 同时覆盖该 workspace 父目录与 per-name `<name>/` 子目录两层（详见 §8.8 R5 修订）。
  2. **acquire** `<workspace>/skills/agent/.create.lock`（默认 timeout 1.0s；超时 → `concurrency_timeout`）。在构造 `FileLock` 之前先按 §3.7.1 的 symlink-defense 校验该 lock 路径不是 symlink（R5 YEL-R5-5）。
...
1. 在构造 `filelock.FileLock(<lock_path>)` **之前**，先做 `if Path(<lock_path>).is_symlink(): raise SkillManageError(error_code="PATH_ESCAPE")`。该检查发生在 acquire 之前，避免任何 fd 真的落到 symlink 目标上。
2. 同款检查覆盖：
...
3. 该检查 TOCTOU window 不可避免（attacker 可在检查后、acquire 前替换为 symlink），但 `O_NOFOLLOW`-equivalent 的 filelock 内部 fd 拿取 + §4.6.1 既有的"resolve-and-relative-to"二次守护构成 defense-in-depth；symlink 预检是第一道闸门。
4. 失败路径不 bump，不持任何下游锁，直接返回 `error_code = "PATH_ESCAPE"`。

§10.6 R5 增补 test gate："planted symlink at `<workspace>/skills/agent/.create.lock` → `PATH_ESCAPE` reject before filelock acquire" 与"planted symlink at `<workspace>/skills/agent/<name>/.lock` → 同款 reject"。

决策 Log #67-A（R5）：lock 文件路径自身的 symlink 校验，与 §4.6.1 SKILL.md 路径校验同款合同。
...
1. **持锁后**（in-proc lock + filelock，§8.6 第 1+2 层已持有）把 `<name>/SKILL.md` 全量读入 string `current`。
...
7. 释放 filelock（第 2 层）→ 释放 in-proc lock（第 1 层），LIFO。返回。
...
> **R5 注脚（YEL-R5-4 — Coherence hygiene）**：本节是 counter 形态的**权威**说明。决策日志 §13 row #55 已被 R5 改写为"R3→R4 演进史"条目，仅记录设计变迁；M3 review 阅读权威 spec 文本以本节 §5.2.1 + §12 carried-forward 为准。
...
**三个候选 wiring approach**（plan-author 选其一并写入 plan §file-map）：

- **Option W1（推荐 — `AgentRunSpec` 字段）**：
  - 在 `AgentRunSpec` dataclass 上加 `runtime_state: RuntimeState | None = None` 字段（带默认值 `None` 保证向后兼容现有 caller，老路径不传即可）。
  ...
  - **优点**：与现有 `AgentRunSpec` 已有的 `max_iterations` / `tools` / etc. 字段同款风格；`AgentRunner` 实例不存状态，仅消费 spec；与 `AgentRunner` 单实例（`SubagentManager.runner = AgentRunner(provider)`）哲学一致。
  - **建议默认选这条**，除非 plan-author 找到硬阻挡。

- **Option W2（`AgentRunner.__init__` 参数）**：
  - `AgentRunner.__init__` 加 `runtime_state: RuntimeState | None = None` keyword-only 参数。
  - `AgentLoop` 在构造 `AgentRunner(provider, runtime_state=self.runtime_state)` 时传入。
  - `_run_core` 读 `self._runtime_state_`（蛇形 + 尾下划线，遵循项目命名规约）做 reset。
  - **缺点**：`AgentRunner` 是**单实例**（`subagent.py:116, 248` 父-子复用），在构造期绑定 `RuntimeState` 引用会让 subagent 与父 agent 的 RuntimeState 生命周期耦合 —— 若 SubagentManager 给 subagent 传**不同** RuntimeState 时需要重新构造 runner，破坏单实例假设。**不推荐**。

- **Option W3（reset callable through spec）**：
  - `AgentRunSpec` 加 `reset_per_iteration: Callable[[], None] | None = None` 字段。
  - `AgentLoop` 把 `lambda: self.runtime_state._runtime_vars.update({"skill_manage.mutations_this_turn": 0})` 之类的闭包塞进去。
  - `_run_core` for-loop 顶部 `if spec.reset_per_iteration is not None: spec.reset_per_iteration()`。
  - **优点**：runner 完全不感知 RuntimeState 字段语义，最大解耦。
  - **缺点**：多一层 callback 间接，未来加更多 per-iteration RuntimeState 操作时回调列表会膨胀。
  - **备选**，仅当 W1 被否决时再考虑。

**spec 不锁行号**：`runner.py:343` 是当前 mainline 的 for-loop 行号，runner.py 仍在演化；plan-author 落地时按当时实际 for-loop 入口为准。**spec 锁的是数据结构 + 注入语义**：reset 必须发生在每 iteration 顶部、必须与现有 `workspace_violation_counts` (`runner.py:337`) 同作用域。

决策 Log #67（R5）：reset-point wiring 三个候选 W1/W2/W3 已命名，W1 推荐；最终选择由 plan-author 在 plan §file-map 决定。
...
M2 在 `create` 路径**不直接** mutate telemetry；`delete` 路径仅 bump tombstone marker，**不**删 entry：
...
- **Option A（推荐 — lift-and-upgrade）**：把 `_atomic_write` 从 `skills_telemetry.py` lift 到 `nanobot/agent/_atomic_io.py` 公共 utility，**同时**把 flag 升级为 `O_WRONLY | O_CREAT | O_TRUNC | O_NOFOLLOW | O_CLOEXEC`（按 §4.6.1 跨平台兼容协议用 `getattr` 兜底）。M1 telemetry 路径 `inherit` 升级后的 flag——对 telemetry 是无害收紧（telemetry tmp 文件本就在 telemetry root 内、本就不该是 symlink 也不该跨 fork 泄漏 fd）。
...
- **Option B（duplicate）**：保留 M1 telemetry 既有 bare flag 不动，`skill_manage.py` 用独立的强化版本；可选但产生两份 atomic-write 实现。Option B 选项下**不需要** R5 上述 back-compat 工作（M1 telemetry 模块零改动）。
- **取舍权交 plan-author**：若 M1 telemetry 升级 flag 风险过高（unforeseen Windows 路径）或 plan-author 不愿承担 R5 三条合同的实施代价，落 Option B；否则首选 Option A 并落实三条合同。任一选项都不影响 M2 自己的耐久性合同。
...
- **Nonce randomness（R5 fix YEL-R5-6 — Security B）**：`<nonce>` **必须**用密码学强随机源 `secrets.token_hex(8)`（生成 16 hex chars）。**禁止**任何可预测来源：`time.monotonic_ns` / `time.time_ns` / `random.randint` / 进程内序列计数器（`itertools.count`）一律 forbidden。理由：Windows 缺 `O_NOFOLLOW`、§4.6.1 走 `is_symlink` 预检 + resolve 兜底——预检与 `os.open` 之间存在 TOCTOU window；攻击者若能预测 tmp 文件名，可在 window 内于该路径预放 symlink 把 tmp 写入重定向到外部目标。`secrets.token_hex` 让 tmp 名不可预测，关闭这条预测路径。
  - 静态扫描 / code review gate：实现源码必须 `import secrets` + `secrets.token_hex(8)`；`grep -E "random|time\.(monotonic|time)_ns|itertools\.count" <atomic_write_module>` 在 nonce 取数行附近不应命中（M2 plan acceptance gate）。
...
决策 Log #57 + 决策 Log #65（lift-vs-duplicate 取舍权交 plan-author）。
...
0. **skill_manage workspace create-lock**（`<workspace>/skills/agent/.create.lock`，filelock；**仅** `create` verb 取，§3.7 R4 RED-1 fix）
...
- **mkdir-on-first-create 覆盖两层**（R5 修订澄清 YEL-R5-1）：
  - **workspace-level**：`<workspace>/skills/agent/` 父目录在 `create` verb 的 §3.7 step 1.5 通过 `os.makedirs(..., exist_ok=True)` 重建（race-safe，幂等）。这是 `<workspace>/skills/agent/.create.lock` 能被 `filelock.FileLock(...).acquire()` 创建的前提。
...
- 若 plan-author 选择 Option B（duplicate），本节 gate 不适用（M1 telemetry 模块零改动）。
...
#### Atomic-write nonce 强随机（R5 YEL-R5-6 — Security B）
...
- assert 实现源码使用 `secrets.token_hex` 而非 `random` / `time` / 序列计数器（静态扫描 / code review gate）。
...
| 55 | 2026-06-11 (R3 → **superseded by R4**) | **历史条目**：rate-cap counter 形态从 R3 4-桶 dict 简化为 R4 单一 int | A: dict（R3 已废） / B: int（R4 采纳） | R3 motivation（per-verb 信号给 M3 Curator 用 create-spam vs delete-spam）保留为 §12 carried-forward（"M3 拿到具体 consumer 时再升级 dict"）；M2 无消费者，单 int 最小化测试网与实施。**当前权威 spec 文本在 §5.2.1**；本行仅作 R3→R4 演进史保留，**不**再视为 active decision。M3 review 直接看 §5.2.1 与 §12 carried-forward，不要再回头读本行 |
...
| 65 | 2026-06-11 (R4) | `_atomic_write` lift-vs-duplicate 取舍权交 plan-author，spec 仅锁 flag 集（O_NOFOLLOW + O_CLOEXEC + 跨平台 fallback） | A: lift-and-upgrade 推荐 / B: duplicate / C: M2 plan 决定 | M1 既有 flag 弱于 §8.5 要求；plan-author 评估 telemetry 升级风险后定 |
...
| 67 | 2026-06-11 (R5) | `_run_core` per-iteration reset 的 RuntimeState 注入路径：3 个候选 wiring 已命名，W1 推荐 | A: W1 `AgentRunSpec.runtime_state` 字段（推荐） / B: W2 `AgentRunner.__init__` 参数 / C: W3 reset callable through spec | W2 让 single-instance runner 与 RuntimeState 生命周期耦合，破坏 SubagentManager 父子复用；W3 多一层间接；W1 与现有 spec 字段风格一致；最终选 plan-author 在 plan §file-map 决定（详见 §5.2.2） |
| 67-A | 2026-06-11 (R5) | filelock 路径自身（`<workspace>/skills/agent/.create.lock` 与 `<name>/.lock`）的 symlink-defense | A: acquire 前 `is_symlink` 预检 reject `PATH_ESCAPE`（采纳） / B: 信任 filelock 内部 fd 路径处理 | 攻击者预置的 lock-path symlink 让 advisory lock 落到 workspace 之外，跨进程互斥失效；预检 + §4.6.1 既有 resolve 兜底构成 defense-in-depth（详见 §3.7.1） |
