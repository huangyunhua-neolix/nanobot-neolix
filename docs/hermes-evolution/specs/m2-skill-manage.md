...
>
> **状态**：设计已锁定（2026-06-11，Q1–Q13 决策全部 approved；R3 patch 已合入：14 RED + 12 YELLOW 全部 resolved；R4 patch 已合入：2 RED + 17 YELLOW 全部 resolved，6-lens 验证 zero outstanding）。
>
...
- **协议**（仅 `create`）：
  ...
  2. **acquire** `<workspace>/skills/agent/.create.lock`（默认 timeout 1.0s；超时 → `concurrency_timeout`）。
...
- **nested-dict key order**：PyYAML 5.1+ 在 `sort_keys=False` 下保留 insertion order；`pyproject.toml` 必须 pin `pyyaml>=5.1`（M2 plan 阶段实施时确认 lock file）。
...
- **Option B（duplicate）**：保留 M1 telemetry 既有 bare flag 不动，`skill_manage.py` 用独立的强化版本；可选但产生两份 atomic-write 实现。
- **取舍权交 plan-author**：若 M1 telemetry 升级 flag 风险过高（unforeseen Windows 路径），落 Option B；否则首选 Option A。任一选项都不影响 M2 自己的耐久性合同。
...
| 55 | 2026-06-11 (R3, **superseded by R4**) | rate-cap counter 形态 | R3: 4-桶 dict / R4: **单一 int** | R4 改写：M2 内无 4-桶消费者；切单 int 简化测试与实施；M3 升级为 dict 时 bump `SkillManageConfig` schema 字段（详见 §5.2.1 R4 修订） |
